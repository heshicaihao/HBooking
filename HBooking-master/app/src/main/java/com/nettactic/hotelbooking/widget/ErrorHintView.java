...
import com.nettactic.hotelbooking.listener.IStrategy;
...
public class ErrorHintView extends RelativeLayout {
	...
	private void init() {
		View.inflate(getContext(), R.layout.widget_hint_view_error, this);
		...
		public void operate() {
			if (loadFailure == null) {
				loadFailure = View.inflate(getContext(),
						R.layout.widget_hint_view_load_failure, null);
		...
		public void operate() {
			if (netError == null) {
				netError = View.inflate(getContext(),
						R.layout.widget_hint_view_load_wifi_failure, null);
	...
	class NoDataError implements IStrategy {
		...
		public void operate() {
			if (noData == null) {
				noData = View.inflate(getContext(), R.layout.widget_hint_view_load_noorder,
						null);
			}
			mContainer.removeAllViews();
		...
		public void operate() {
			if (loadingdata == null) {
				loadingdata = View.inflate(getContext(),
						R.layout.widget_hint_view_load_loading, null);
			}
