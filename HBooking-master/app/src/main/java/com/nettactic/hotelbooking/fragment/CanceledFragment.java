...
public class CanceledFragment extends BaseFragment  implements OnRefreshListener {
...
//        mIsNoNull = SharedpreferncesUtil.getOrder(getContext());
    ...
    private void initView(View mView) {
        ...
        mAdapter = new OrderListAdapter(getActivity(), getContext(),2);
    ...
    private void initData() {
        ...
        if (mUser.isIs_login() && mIsNoNull) {
            String order_listfilepath = FileUtil.getFilePath(
                    MyConstants.ORDERLIST, MyConstants.ORDER_LIST_CANCELED,
                    MyConstants.TXT);
            ...
            if (!fileIsExists) {
                ...
                mGoodsListStr = FileUtil.readFile(MyConstants.ORDERLIST,
                        MyConstants.ORDER_LIST_CANCELED, MyConstants.TXT);
                try {
    ...
    private void getOrderInfo(final boolean flag) {
        String json = AssetsUtils.getJson(getContext(), "order_list_canceled.txt");
